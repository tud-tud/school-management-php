...
if (isset($_SESSION['admin_id']) && isset($_SESSION['role'])) {
    if ($_SESSION['role'] == '1') {
        ...
        if (isset($_POST['admin_pass']) && isset($_POST['new_pass']) && isset($_POST['c_new_pass']) && isset($_POST['r_user_id'])) {
            ...
            if (empty($admin_pass)) {
                ...
                header("Location:../register-office-edit.php?perror=$em&$data");
                exit;
                ...
                $sql = "UPDATE register_office SET password= ? WHERE r_user_id=? ";

                $stmt = $conn->prepare($sql);
                $stmt->execute([$new_pass, $r_user_id]);

                $sm = "The password has been change successfully!";
                header("Location:../register-office-edit.php?psuccess=$sm&$data");
                exit;
            }
        } else {
